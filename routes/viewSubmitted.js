...
async function viewAttempts(courseID,year,semester,res){

  let selectDBSQL = `USE LMS;`;
  await seedData(selectDBSQL);

  let resource_query = `select * from Attempts where course_id=${courseID} and year_offered=${year} and sem_offered="${semester}";`;
